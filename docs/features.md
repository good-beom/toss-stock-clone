...
WatchlistItem이 각자 `useQuery`를 갖는 N+1 패턴 대신, 페이지 레벨에서 `watchlistQuotesOptions`로 모든 심볼을 병렬 fetch한다. 폴링 타이머가 N개가 아닌 1개만 생성된다.

`Promise.allSettled`를 사용해 개별 종목 조회가 실패해도 나머지 종목은 정상 표시된다. 결과는 `Map<symbol, StockQuote>`로 변환해 O(1) 조회한다.
...
두 곳에 배치되어 있으며 동일한 Zustand 스토어를 공유하므로 어디서 바꿔도 즉시 전체에 반영된다.
...
설정은 Zustand `persist`로 localStorage에 저장되어 새로고침 후에도 유지된다.
...
모든 Route Handler에는 `revalidate` 상수가 설정되어 있어 Next.js가 응답을 ISR 캐시로 관리한다.

| 엔드포인트 | revalidate |
|-----------|-----------|
| `/api/stock/:symbol` | 10초 |
| `/api/market/actives` | 60초 |
| `/api/stock/:symbol/candles` | 60초 |
| `/api/forex/rate` | 3600초 (1시간) |
| `/api/search` | 300초 (5분) |
